...
if __name__ == "__main__":
    ...
    for SubjLoop in range(0, len(SubjFolders)):  # first two items are garbage
        ...
        if settings["CalculateHJC"] == 'Yes':
            print('Adding HJCs')
            filesAddHJC = [trial['name'] + '.c3d' for trial in Trials]
...
#           findHJC(Trials[StaticTrial]["name"]+'.trc', Trials[StaticTrial]["folder"]+'/',filesAddHJC)
        ...
        if settings["ConvertForces"] == 'Yes':
            for j in range(len(Trials)):                   
                ...
                convertFPdata_OpenSim(Trials[j]["files"]["c3d"], Trials[j]["folder"], 0, settings);
        ...
        # Load GRF
        Trials[i]["GRF"] = LoadGRF([os.path.join(Trials[i]["folder"], 'OpenSim', Trials[i]["files"]["OpenSimGRF"])], 0)
        # Define vGRF columns
        Cols = ['ground_force_vy', '1_ground_force_vy']
